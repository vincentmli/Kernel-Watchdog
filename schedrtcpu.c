/*
 * schedrtcpu.c - schedrtcpu 
 * schedrtcpu run on specific CPU with realtime priority
 * it it consumes almost 100% sinle cpu

 * Vincent Li <mchun.li@gmail.com>	21 Nov 2011
 *
 * steal most of the code from taskset.c from util-linux, need to work on -c argument
 *
...
void go_fifo() {
        ...
        int result = sched_setscheduler(0, SCHED_FIFO, &sched_param);
        if (result < 0)
                printf("Failed to set fifo priority.\n");
#endif
}
void busyloop() {
        volatile int n = 0, t=0;
        volatile int x = 1;
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv, "+chV", longopts, NULL)) != -1) {
		int ret = 1;
	...
	if (sched_setaffinity(0, new_mask_byte_size,
	...
	if (sched_getaffinity(0, cur_mask_byte_size,
					(cpu_set_t *)cur_mask->maskp) < 0) {
		perror("sched_getaffinity");
		fprintf(stderr, "failed to get  affinity.\n" );
		return 1;
	}

	if (c_opt) {
		printf("new affinity list: %s\n", cpuset_to_cstr(cur_mask, cstr));
	}
	else { 
		printf("pid %d affinity mask: %s\n", getpid(), cpuset_to_str(cur_mask, mstr));
	} 
